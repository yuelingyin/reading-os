...
export default async function BookDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!book) notFound()
  const { data: reviews } = await supabase.from('chapter_reviews').select('*').eq('book_id', id).eq('user_id', user.id).order('chapter_number', { ascending: true })
  const { data: actionItems } = await supabase.from('action_items').select('*').eq('book_id', id).eq('user_id', user.id).order('created_at', { ascending: true })
  const { data: extendedReadings } = await supabase.from('extended_reading').select('*').eq('book_id', id).eq('user_id', user.id).order('created_at', { ascending: true })
  ...
  return (
    <div className="min-h-screen bg-white text-black p-6 md:p-12">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="mb-6">
          <h2 className="text-lg font-semibold mb-4">章节复盘 <span className="text-gray-400 font-normal">{reviews?.length || 0} 章</span></h2>
        ...
        <div className="mb-24">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href={`/books/${id}/extend`}><Button variant="outline" size="sm"><Plus className="w-4 h-4 mr-1" />添加延伸线索</Button></Link>
          </div>
        ...
        <div className="fixed bottom-6 right-6 flex flex-col gap-3 items-end">
          {isCompleted ? <div className="bg-gray-900 text-white px-4 py-2 rounded-full text-sm font-medium shadow-lg">本书已结案</div> : (<>
            <Link href={`/books/${id}/action`}><Button size="sm" className="bg-gray-800 text-white hover:bg-gray-700 shadow-lg rounded-full"><Flag className="w-4 h-4 mr-1" />完结并生成行动清单</Button></Link>
            <Link href={`/books/${id}/review`}><Button size="lg" className="bg-black text-white hover:bg-gray-800 shadow-lg rounded-full"><Plus className="w-5 h-5 mr-2" />添加章节复盘</Button></Link>
          </>)}
        </div>
      </div>
    </div>
